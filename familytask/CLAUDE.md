...
- **AI assistant**: `POST /api/assistant` (token-protected) sends the raw message to a local Ollama model (`docker-compose.yml` service `ollama`, OpenAI-compatible API, no key needed — GitHub Models, used initially, was permanently retired 2026-07-30). It declares a `ajouter_tache` function-calling tool (`titre`, `personne`); when the model returns a `tool_calls` entry, `main.py` decodes the JSON-string `arguments`, resolves `personne` case-insensitively against `Member.name` within the caller's family, and actually creates the `Task` row (same admin-only-if-assigning-to-someone-else rule as `create_task`). Before calling the model at all, `trouver_lien_ambigu` scans the *raw* message for a `lien` (e.g. "fille") shared by 2+ family members and short-circuits with a disambiguation question instead of guessing.
